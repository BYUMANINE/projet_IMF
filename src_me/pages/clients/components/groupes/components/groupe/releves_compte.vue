    ...
    return {
      ...
      defaultValues: {
        filters: {
          ...
          membre: { id: 'ALL', nom_complet: 'Tous les membres' }
        },
        data: {
          repport: {},
          operations: [],
  ...
  methods: {
    ...
    filterComptes (val, update, abort) {
      let donnees = JSON.stringify({
        ...
        signe_length: '>=',
        with_length: '14',
        id_groupe: this.type_select === 'COMPTE'
          ? (this.groupe ? this.groupe.id : null)
          : null
      })
    ...
    filterMembres (val, update, abort) {
      let donnees = JSON.stringify({
        ...
        id_agent: this.user.id,
        id_agence: this.user.agence.id,
        id_groupe: this.groupe ? this.groupe.id : null,
        only: 'ALL'
      })
      ...
      this.$axios.post(url, this.$helper.objectToform({ 'data': donnees })).then((infos) => {
        update(() => {
          this.membres = [...[this.defaultValues.filters.membre], ...infos.data.records]
        })
      }).catch(() => {
        update(() => {
          this.membres = [this.defaultValues.filters.membre]
        })
      })
    },
    ...
    getDatas () {
      if (this.$helper.isNotEmpty(this.filterListe.devise) &&
        this.$helper.isNotEmpty(this.filterListe.membre) &&
        ...
        const donnees = JSON.stringify({
          ...
          id_membre_gr: this.filterListe.membre.id,
