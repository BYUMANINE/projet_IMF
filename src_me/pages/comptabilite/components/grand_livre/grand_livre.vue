              ...
              <table class="table head-bold hover table-striped  table-colored-head">
                ...
                <tbody style="font-size:11.5px">
                  <tr>
                    ...
                    >REPPORT</td>
                    <td class="text-right text-blue bg-blue-1 text-bold">{{$helper.formatMoney(data.repport.montant)}} S{{data.repport.solde}}</td>
                    <td
                      colspan="2"
                  ...
                  >
                    ...
                    <td class="text-bold text-right">{{row.operation == 'D' ? $helper.formatMoney(row.montant) : '0,00'}}</td>
                    <td class="text-bold text-right">{{row.operation == 'C' ? $helper.formatMoney(row.montant) : '0,00'}}</td>
                    <td :class="`text-bold text-right ${row.solde < 0 ?'bg-red-1 text-red':'bg-blue-1 text-primary'}`">{{$helper.formatMoney(row.solde)}}</td>
    ...
    return {
      ...
      defaultValues: {
        filters: {
          devise: 'CDF',
          compte: null,
          compte_cp: { id: 'ALL', devise: '', indice: 'ALL', intitule: 'Tous les comptes' },
          produit: { id: 'ALL', devise: '', indice: 'ALL', designation: 'Tous les produits' }
        },
        data: {
          repport: {},
          operations: [],
  ...
  methods: {
    ...
    filterProduits (val, update, abort) {
      ...
      this.$axios.post(url, this.$helper.objectToform({ 'data': donnees })).then((infos) => {
        update(() => {
          this.produits = [
            ...[this.defaultValues.filters.produit],
            ...infos.data.records]
        })
      }).catch(() => {
        update(() => {
          this.produits = [this.defaultValues.filters.produit]
        })
      })
    },
    onDeviseSelected (e) {
