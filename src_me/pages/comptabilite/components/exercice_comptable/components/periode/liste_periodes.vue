<template>
  <q-page style="min-height:0">
    ...
    <list-menu-options>
      ...
      <menu-option
        text="Ouverture période"
        icon="open.png"
        @option-clicked="ouverturePeriode()"
      />
        ...
        <div class="col">
          <div class="panel-primary shadow-1 ">
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <q-item-section>Clôturer cette periode ?</q-item-section>
                        </q-item>
                      </q-list>
                    </q-menu>
            ...
            <myPagination
              ...
              idPagination="paginationproduit"
              @onPaginate="getDatas()"
            />
          </div>
        </div>
  ...
  methods: {
    ...
    getDatas (url = null) {
      if (this.$helper.isNotEmpty(this.filterListe.peer_page)) {
        ...
        url = url || `${this.URLS.BASE_URL}/Exercice/getAllPeriodes`
        ...
        this.$axios.post(url, this.$helper.objectToform({ 'data': donnees })).then((infos) => {
          ...
          if (infos.data && infos.data.records.length > 0) {
            ...
            let el = document.querySelector('.paginationproduit')

            if (el) {
              el.innerHTML = infos.data.pagination
              document.querySelectorAll('.paginationproduit a').forEach(el => {
                el.onclick = e => {
                  e.preventDefault()
    ...
    ouverturePeriode () {
      this.$q.dialog({
        ...
        message: `Souhaitez-vous vraiment ouvrir une nouvelle période ?`,
        cancel: 'Non',
        ok: 'Oui',
        persistent: true
      }).onOk(() => {
        ...
        this.$q.loading.show()
        let url = `${this.URLS.BASE_URL}/Exercice/ouverturePeriode/`
    ...
    cloturePeriode (row) {
      this.$q.dialog({
        dark: this.$q.dark.isActive,
        title: 'Clôture période en cours...',
        message: `Souhaitez-vous vraiment cloturer cette période ?`,
        cancel: 'Non',
        ok: 'Oui',
        persistent: true
      }).onOk(() => {
        ...
        this.$q.loading.show()
        let url = `${this.URLS.BASE_URL}/Exercice/cloturePeriode/`
