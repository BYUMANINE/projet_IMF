<template>
  <q-page style="min-height:0">
        ...
        <div class="col">
          <div class="panel-primary shadow-1 ">
              ...
              <table class="table head-bold hover table-striped table-colored-head">
                ...
                <tbody style="font-size:12px">
                  ...
                  >
                    ...
                    <td class="text-bold text-left">{{ row.intervalle_str }}</td>
                    <td class="text-center">{{row.min_jour }}</td>
                    <td class="text-center">{{row.max_jour }}</td>
                    <td class="text-center">{{row.taux_provision }} %</td>
                    ...
                    >
                      ...
                      >
                        ...
                        <q-item
                          ...
                          v-ripple
                          v-close-popup
                          @click="selectedIntervalle=row;isAdd=false;showDlgIntervalle=true"
                        >
            ...
            <myPagination
              ...
              idPagination="paginationIntervalleCredit"
              @onPaginate="getDatas()"
            />
          </div>
        </div>
    ...
    <search
      ...
      :filters="[
        {value:'credit_intervalle_retard.min_jour|%%',label:'Par min'},
        {value:'credit_intervalle_retard.min_jour|%%',label:'Par max'},
        {value:'credit_intervalle_retard.taux_provision|%%',label:'Par taux provision'}
      ]"
      @onSearch="(e)=>{searchOperation=e;this.getDatas()}"
    />

    <nouveauIntervalle
      v-model="showDlgIntervalle"
      :selectedIntervalle="selectedIntervalle"
      @onFinish="getDatas()"
    />
  </q-page>
</template>
<script>
import nouveauIntervalle from './nouvel_intervalle.vue'

export default {
  name: 'liste_intervalle_retard',
  data () {
    return {
      /* URLS AND  USER */
      URLS: {},
  ...
  methods: {
    ...
    getDatas (url = null) {
      ...
      url = url || `${this.URLS.BASE_URL}/Intervalle_retard/getAllIntervalleReatrds`
      ...
      this.$axios
        ...
        .then(infos => {
          ...
          if (infos.data && infos.data.records.length > 0) {
            ...
            let el = document.querySelector('.paginationIntervalleCredit')

            if (el) {
              el.innerHTML = infos.data.pagination
              document.querySelectorAll('.paginationIntervalleCredit a').forEach(el => {
                el.onclick = e => {
                  e.preventDefault()
    ...
    supprimer (row) {
      this.$q.dialog({
        ...
        this.$q.loading.show()
        let url = `${this.URLS.BASE_URL}/Intervalle_credit/delete/`
