        ...
        <div class="col-xs-12 col-sm col-md col-lg col-xl">
          ...
          >
            <q-card-section class="row q-pa-sm q-col-gutter-sm">
              ...
              <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                <input-label>Grouper par type comptes</input-label>
                <q-select
                  transition-show="scale"
                  transition-hide="scale"
                  square
...
export default {
  ...
  data () {
    return {
      ...
      compte_default: { id: 'ALL', indice: 'ALL', intitule: 'Tous les comptes' }
    }
  },
  props: {
    value: Object
  ...
  beforeMount () {
    ...
    this.$vue.set(this.filtre, 'compte', this.ocompte_default)
    this.$vue.set(this.filtre, 'groupe_indice', this.groupes_indices[0])

    this.filtre.search = null
  },
  mounted: function () {
  },
  destroyed: function () {
  ...
  methods: {
    ...
    filterComptes (val, update, abort) {
      ...
      this.$axios.post(url, this.$helper.objectToform({ 'data': donnees })).then((infos) => {
        update(() => {
          this.comptes = [...[this.compte_default], ...infos.data.records]
        })
      }).catch(() => {
        update(() => {
