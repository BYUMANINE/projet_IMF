<template>
  <q-page class="filly-auto">
                  ...
                  <div class="ba overflow-hidden panel-primary">
                    ...
                    <div class="q-px-md q-pt-sm q-pb-md">
                      <div class="row q-col-gutter-sm">
                        ...
                        <div class="col-12">
                          <input-label>Mode de retrait</input-label>
                          <q-select
                            transition-show="scale"
                            transition-hide="scale"
                            square
                              ...
                              >
                                <q-item dense>
                                  <q-item-section avatar>
                                    ...
                                    >
                                      <q-img
                                        :src="selectedMandataire ? (!!selectedMandataire.photo ?  `${URLS.IMG_MANDATAIRE}/${selectedMandataire.photo}` :'statics/images/icone/avatar.png') : 'statics/images/icone/avatar.png'"
                                        spinner-color="blue"
                                        spinner-size="15px"
                                        class="panel-primary"
                                  ...
                                  <q-item-section>
                                    <q-item-label>{{selectedMandataire? selectedMandataire.nom_complet : 'N/A'}}</q-item-label>
                                    <q-item-label
                                      caption
                                      lines="2"
                                    >{{selectedMandataire? selectedMandataire.sexe : 'N/A'}}</q-item-label>
                                  </q-item-section>
                                  <q-item-section side>
                                    <q-btn
                                      color="blue-1"
                    ...
                    <div class="q-px-md q-pt-sm q-pb-md">
                      <div class="row q-col-gutter-sm">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md col-lg col-xl">
                          ...
                          <q-input
                            :readonly="modeRetrait === 'MANDATAIRE'"
                            square
                            outlined
                            dense
                            hide-bottom-space
                        ...
                        <div
                          v-if="modeRetrait==='CHEQUE'"
                          class="col-xs-12 col-sm-12 col-md col-lg col-xl"
                        >
                          <input-label required>Numéro du chèque</input-label>
                          <q-input
                            square
                            outlined
                            dense
                            ...
                            lazy-rules
                            :rules="[ val => _isPieceRequired(val) || 'Entrer le numero du chèque']"
                            maxlength="150"
                          />
                        </div>
    ...
    <selectMandataire
      v-model="showDlgMandataire"
      :mandataire="selectedMandataire"
      :mandataires="mandataires"
      :membre="membre"
      :URLS="URLS"
      :user="user"
      @onFinish="onMandataireSelected"
    />
...
import selectMandataire from './select_mandataire.vue'
...
export default {
  ...
  data () {
    return {
      ...
      modeRetrait: 'MEMBRE',
      typesBeneficiaires: [
        { value: 'MEMBRE', label: 'Le membre lui-même' },
        { value: 'MANDATAIRE', label: 'Le mandataire au compte' },
        { value: 'CHEQUE', label: 'Par procuration' }
      ],
      membres: [],
      mandataires: [],
      comptes: [],
  ...
  watch: {
    ...
    'modeRetrait': {
      handler (newValue, oldValue) {
        ...
        this.$vue.set(this.operation, 'piece', null)

        if (newValue !== 'MANDATAIRE') {
          if (newValue === 'MEMBRE') {
            this.$vue.set(this.operation, 'initiateur', this.membre.nom_complet)
          }
        }
      },
      deep: false
    ...
    _libelle: {
      get () {
        if (this.operation.default_libelle) {
          this.$vue.set(this.operation, 'libelle', `Retrait de ${this.$helper.formatMoney(this.operation.montant) || '0,00'} ${this.operation.devise || ''} dans le compte ${this.compte ? this.compte.indice : ''} du membre ${this.membre ? this.membre.nom_complet : ''}. Bénéficiaire : ${this.operation.initiateur || 'Non défini'}`)
        }
  ...
  methods: {
    _validateRulesMandataire () {
      if (this.modeRetrait === 'MANDATAIRE') {
    ...
    _isPieceRequired (val) {
      if (this.modeRetrait === 'CHEQUE') {
        return !!val
      } else {
    ...
    onMandataireSelected (e) {
      if (e) {
        this.selectedMandataire = e
        this.$vue.set(this.operation, 'initiateur', `${e.nom_complet}`)
      }
    },
    ...
    saveForm () {
      ...
      this.$q.dialog({
        ...
        const dt = {
          ...
          id_mandataire: this.selectedMandataire ? this.selectedMandataire.id : null,
...
<style>
.border-divider .col:first-child {
