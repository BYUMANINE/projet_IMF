  ...
  <q-page style="min-height:0">
    ...
    <list-menu-options contentStyle="top: 120px">
      ...
      <menu-option
        ...
        tooltip="Alt + A"
        icon="add.png"
        @option-clicked="selectedSecteur=null;isAdd=true;showDlgSecteur=true;"
      />
      <menu-option
        ...
        <div class="col">
          <div class="panel-primary">
                    ...
                    >
                      ...
                      >
                        <q-item
                          ...
                          v-ripple
                          v-close-popup
                          @click="selectedSecteur=row;isAdd=false;showDlgSecteur=true"
                        >
                        ...
                        >
                          ...
                          <q-item-section>{{row.status=='ACTIF'?'Désactiver ce secteur ?':'Activer ce secteur ?'}}</q-item-section>
                        </q-item>
                      </q-list>
                    </q-menu>
                  ...
                  <template v-if="!loading && data.length===0">
                    <tr class="panel-primary">
                      <td :colspan="7">
                        <nondata
                          v-if="!loading"
                          title="Aucun secteur enregistré"
                          @actualiser="getDatas()"
                        />
                      </td>
                    </tr>
            ...
            <myPagination
              ...
              idPagination="paginationsecteur"
              @onPaginate="getDatas()"
            />
          </div>
        </div>
    ...
    <search
      ...
      :filters="[
        {value:'designation|%%',label:'Par intitulé du secteur'},
        ]"
...
<script>
import nouveauSecteur from './nouveau_secteur.vue'
export default {
  name: 'liste_secteur',
  data () {
    return {
  ...
  methods: {
    ...
    getDatas (url = null) {
      if (this.$helper.isNotEmpty(this.filterListe.peer_page)) {
        ...
        url = url || `${this.URLS.BASE_URL}/Secteur/getAllsecteurs`
        ...
        this.$axios.post(url, this.$helper.objectToform({ 'data': donnees })).then((infos) => {
          ...
          if (infos.data && infos.data.records.length > 0) {
            ...
            let el = document.querySelector('.paginationsecteur')

            if (el) {
              el.innerHTML = infos.data.pagination
              document.querySelectorAll('.paginationsecteur a').forEach(el => {
                el.onclick = e => {
                  e.preventDefault()
    ...
    supprimer (row) {
      this.$q.dialog({
        ...
        this.$q.loading.show()
        let url = `${this.URLS.BASE_URL}/Secteur/delete/`
    ...
    enableOrDisable (row) {
      this.$q
        .dialog({
          ...
          message: `Souhaitez-vous ${row.status === 'ACTIF' ? 'désactiver' : 'activer'} ce secteur ?`,
          cancel: 'Non',
          ok: 'Oui',
          persistent: true
        })
        .onOk(() => {
          ...
          this.$q.loading.show()
          let url = `${this.URLS.BASE_URL}/Secteur/enableOrDisable/`
